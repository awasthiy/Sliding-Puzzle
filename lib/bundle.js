...
/***/ function(module, exports, __webpack_require__) {
	...
	View.prototype.setupBoard = function (domELement, contentArray) {
	  for (var rowIdx = 0; rowIdx < this.game.gridSize; rowIdx++) {
	    var $ul = $('<ul>');
	
	    for (var colIdx = 0; colIdx < this.game.gridSize; colIdx++) {
	      var num = contentArray[rowIdx * this.game.gridSize + colIdx];
	      var $li;
	      if (num === null) {
	        $li = $('<li></li>');
	        if (domELement !== this.$gridSolution) $li.addClass('empty');
	      } else {
	        $li = $('<li value=' + num + '></li>');
	      }
